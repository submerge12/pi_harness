...
`AgentHarness` loop. JSONL sessions are persistent. The default user-memory store is currently
in-process; persistent user memory, MCP, durable mid-run resume, OpenTelemetry export, streaming, and
a generic parallel plan executor are not presented as shipped features.
