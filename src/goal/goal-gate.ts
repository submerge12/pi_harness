...
import { completeSimple, getEnvApiKey, type Api, type AssistantMessage, type Model, type SimpleStreamOptions } from "@earendil-works/pi-ai";
...
export function createModelGoalJudge(options: { model: Model<Api>; streamOptions?: SimpleStreamOptions }): GoalGateOptions["judge"] {
	...
	return async ({ signal, ...data }) => {
		const message = await completeSimple(options.model, {
			systemPrompt: [
