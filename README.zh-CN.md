...
当前有 500+ 行为测试，并包含驱动真实 PI `AgentHarness` 的 integration tier，同时作为固定
pi-agent-core 版本的升级门禁。JSONL session 已经持久化。MCP server 可以作为 stdio 工具源接入。
默认用户 memory store 仍是进程内实现；持久用户 memory、运行中持久恢复、OpenTelemetry 导出、
流式事件和通用并行 plan executor 不作为已交付功能介绍。

AOH 已评审的 `PI-HARDENING` 后续计划将补齐 production-path effect authorization、attempt 级
写权限、逻辑副作用幂等和持久化的精确模型资格证明；在当前 AOH Control Room board 关闭前不会启动。
