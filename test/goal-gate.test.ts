...
describe("GenericHarness goal integration", () => {
	...
	it("uses a fresh tool-free model context and rejects prose pretending to be a verdict", async () => {
		const complete = vi.mocked(ai.completeSimple).mockResolvedValueOnce(response('{"status":"complete","reason":"Verified"}')).mockResolvedValueOnce(response("Everything is done."));
		const judge = createModelGoalJudge({ model: resolveHarnessModel(resolveHarnessConfig({})) });
